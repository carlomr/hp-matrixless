...
namespace Step27
{
  ...
  private:
    ...
    void postprocess_uniform_h (const unsigned int cycle);
    void error(const int cycle);

    void error_table()const;
    void find_periodic_boundaries();
    void boundary_term(FullMatrix<double> &lhs_mat,
			Vector<double> &rhs_vec,
			const FEValuesBase<dim> &fe,
			const std::vector<double> &bound_values, 
    ...
    //MyFun::MyRegularExactSolution<dim> exact_solution;
    ...
    Vector<double> DG_error;
    // è uno std::vector perché dealii:Vector non supporta int (si potrebbe farlo ma non ho voglia di leggerecome)
    std::vector<unsigned int> ndof;
    std::vector<std::pair<boundary_face<dim>,boundary_face<dim>>> matches; 
    unsigned int neighbor_of_neighbor_boundary(typename hp::DoFHandler<dim>::cell_iterator cell,
					    typename hp::DoFHandler<dim>::face_iterator face);
  };
  ...
  LaplaceProblem<dim>::LaplaceProblem ()
    ...
    periodic(false),
    num_cycles(26),
    max_degree (2+num_cycles),//TODO:spostarne la definizione fuori
    L2_error(num_cycles),
    DG_error(num_cycles),
    ndof(num_cycles),
    matches()
  {
    ...
    for (unsigned int idx=0; degree<=max_degree; ++idx)
    {
	degree = ceil(.30*idx+2);
	fe_collection.push_back (FE_DGQArbitraryNodes<dim>(QGauss<1>(degree+1)));
	if (idx == 0)
	{
	    quadrature_collection.push_back(QGauss<dim>(degree+20)); 
	    //quadrature_collection.push_back(QGaussJacobi<dim>(degree+30, 0, 0 )); //TODO: in ognuno dei quattro quadrati è diverso: come li scelgo? E' un bel casino
	    //quadrature_collection.push_back (QGaussOneOverR<dim>(degree+30, exact_solution.get_singularity(), true));
	}
	else
	{
			    ...
			    const double & penalty)
  {
      ...
      //std::cout << "face_term: " << cell->index() << "<--->" << neighbor->index() << std::endl;
      ...
      LocalIntegrators::Laplace::ip_matrix(cell_matrix,
	      ...
	      fe_neighbor,
	      penalty);
      //std::cout << "end face_term: " << cell->index() << "<--->" << neighbor->index() << std::endl;
  }
  ...
  template<int dim>
  void LaplaceProblem<dim>::find_periodic_boundaries()
  {
      //std::vector<typename hp::DoFHandler<dim>::cell_iterator> cells_at_boundary;
      //cells_at_boundary.reserve(30); //TODO: find a number
      std::vector<boundary_face<dim>> bf;
      bf.reserve(40);
	...
	for (unsigned int face_no=0; face_no<GeometryInfo<dim>::faces_per_cell; ++face_no)
	{
	    ...
	    if(face->at_boundary())
	    {
		//std::cout << cell->index() << ", " << face->index()<< std::endl;
		bf.push_back(boundary_face<dim>( cell, face, face_no));
	    }
	}

      matches.reserve(ceil(bf.size()/2));

      for (auto face1 =  bf.begin(); face1!= bf.end(); ++face1)
	  for (auto face2 = std::next(face1); face2 != bf.end(); ++face2)
	  {
	  
	      if(matching_faces(face1->face, face2->face))
	      {
		  face1->cell->set_neighbor(face1->side, face2->cell); //va ma non va bene
		  face2->cell->set_neighbor(face2->side, face1->cell); // perche non imposta un po' di altre cose // perche non imposta un po' di altre cose
		  matches.push_back(std::make_pair(*face1, *face2));
		  break;
	      }
	  }
      ...
      for (auto i : matches)
      {
	  //std::cout << i.first.cell->index() << " ( " << i.first.face->index() << " ) -- ( " << i.second.face->index() << " ) " << i.second.cell->index() << std::endl;  
	  //std::cout << i.first.cell->index() << " ( " << i.first.face->index() << " ) -- ( " << i.first.cell->neighbor(i.first.side)->face(i.second.side)->index() << " ) " << i.first.cell->neighbor(i.first.side)->index() << std::endl;  
      }
  ...
  template <int dim>
  unsigned int LaplaceProblem<dim>::
  neighbor_of_neighbor_boundary(typename hp::DoFHandler<dim>::cell_iterator cell,
					    typename hp::DoFHandler<dim>::face_iterator face)
  {
      for(auto i: matches)
      {
	  //std::cout << "niegh of neigh_" << cell->index() << ",  " << i.first.cell->index() << "__" <<( i.first.cell == cell) << std::endl;
	  //std::cout << "niegh of neigh_" << face->index() << ",  " << i.first.face->index() << "__" <<( i.first.face== face) << std::endl;
	  if(i.first.cell==cell && i.first.face == face)
	      return i.second.side;

	  if(i.second.cell==cell && i.second.face == face)
	      return i.first.side;
      }
  ...
  void LaplaceProblem<dim>::assemble_system ()
  {
	...
	std::cout << *local_dof_indices.begin() << "->" << *(local_dof_indices.end()-1) << std::endl;
	...
	//if(on_singularity)
	//{
	    //double alpha = exact_solution.get_alpha();
	    //for(auto i =0; i<fe_values.n_quadrature_points; ++i)
		//rhs_values[i]  = -alpha*(alpha-2+dim);
	//}
	//else
	    exact_solution.laplacian_list (fe_values.get_quadrature_points(), rhs_values);

	//cell element
	LocalIntegrators::Laplace::cell_matrix(cell_matrix, fe_values);
	//LocalIntegrators::L2::mass_matrix(cell_matrix, fe_values);
	//rhs cell element
	...
	for (unsigned int face_no=0; face_no<GeometryInfo<dim>::faces_per_cell; ++face_no)
	{
		...
		Assert(!periodic, ExcInternalError());
		...
		//std::cout << "boundary: "  << cell->index()<< " " << face_no  << std::endl;
	    }
	    else
	    {
		...
		if (face->has_children())
		{
			...
			//std::cout << "main-subface: " << cell->index() << ", " << neighbor->index() << ", " << (!cell->neighbor_is_coarser(face_no) || (periodic && face->at_boundary())) << ", " << cell->level() << ", " << neighbor->level()<< std::endl;
		...
		else
		{
		    ...
		    if ((!cell->neighbor_is_coarser(face_no) 
				//|| (periodic && face->at_boundary()) 
				)&&
			    (neighbor->index() > cell->index() ||
			     (neighbor->level() < cell->level() &&
			      neighbor->index() == cell->index())))
		    {
			//std::cout << "main: " << cell->index() << ", " << neighbor->index() << ", " << (!cell->neighbor_is_coarser(face_no) || (periodic && face->at_boundary())) << ", " << cell->level() << ", " << neighbor->level()<< std::endl;
		    //std::cout << "main: " << cell->index() << ", " << neighbor->index() << ", " << cell->neighbor_is_coarser(face_no)<< ", " << cell->level() << ", " << neighbor->level()<< std::endl;
			...
			//if(!(periodic && face->at_boundary()))
			    neighbor2=cell->neighbor_of_neighbor(face_no);
			//else
			    //neighbor2=neighbor_of_neighbor_boundary(cell, face);
			...
			//std::cout << "neighboring:"<< cell->index() << "--" << face_no << "-->" << neighbor->index() << "( "  << neighbor2 <<" )"<< std::endl;
  ...
  void LaplaceProblem<dim>::solve ()
  {
    ...
    umfp.initialize(system_matrix); 
    umfp.vmult(solution, system_rhs);
  }



  template <int dim>
  void LaplaceProblem<dim>::error (const int cycle )
  {
      const UpdateFlags update_flags = update_values
                                     | update_gradients
                                     | update_quadrature_points
      ...
      double this_L2_error = 0.;
      double this_DG_error = 0.;

      //typename hp::DoFHandler<dim>::active_cell_iterator
      //cell = dof_handler.begin_active(),
      //endc = dof_handler.end();
      Vector<double> cell_L2err(triangulation.n_active_cells());
      Vector<double> cell_DGerr(triangulation.n_active_cells());
      unsigned int idx = 0;
      for (auto cell :  dof_handler.active_cell_iterators())
      {
	  const unsigned int   dofs_per_cell = cell->get_fe().dofs_per_cell;
	  ...
	  {
	      cell_L2err(idx) += (solution_values[k] - exact_values[k])*(solution_values[k] - exact_values[k])*fe_values.JxW(k);
	      cell_DGerr(idx) += (solution_gradients[k] - exact_gradients[k])*(solution_gradients[k] - exact_gradients[k])*fe_values.JxW(k);
	  }
	  this_L2_error += sqrt(cell_L2err(idx));
	  this_DG_error += sqrt(cell_DGerr(idx));
	  ++idx;

      }
      L2_error(cycle) = this_L2_error;
      DG_error(cycle) = this_DG_error;
      ndof[cycle] = dof_handler.n_dofs();

      DataOut<dim,hp::DoFHandler<dim> > data_out;

      data_out.attach_dof_handler (dof_handler);

      data_out.add_data_vector (cell_L2err, "L2_error");
      data_out.add_data_vector (cell_DGerr, "DG_error");
  ...
  template<int dim>
  void LaplaceProblem<dim>::error_table()const
  {
      std::ofstream out("out.m");
      out << "errors = [";
      //out << "N\t err L2\t\t err DG" << std::endl 
		//<< "---------------------------------" << std::endl ;
      for(unsigned int i=0; i< ndof.size(); ++i)
      {
	  out << ndof[i] << "\t" << L2_error(i) << "\t"
	      << DG_error(i) <<";..." <<std::endl;
      }
      //out << "---------------------------------" << std::endl ;
      out << "];semilogy(sqrt(errors(:,1)),errors(:,2),'ro-', sqrt(errors(:,1)), errors(:,3),'o-')" << std::endl;
      out.close();
  }
...
/*      // Let us start with computing estimated error and smoothness indicators,*/
      ...
      std::ofstream out_matrix("matrice"+Utilities::int_to_string(cycle,2)+".m");
      system_matrix.print(out_matrix);
      out_matrix.close();
  }
  template <int dim>
      void LaplaceProblem<dim>::postprocess_uniform_h (const unsigned int cycle)
      {
	  {
	      Vector<float> fe_degrees (triangulation.n_active_cells());
	      {
	  ...
	  for(auto cell : dof_handler.active_cell_iterators())
	  {
	      //if(++idx%2==0)
		  cell->set_refine_flag ();
	      //else if(cell->active_fe_index() < fe_collection.size()-1)
		  //cell->set_active_fe_index(cell->active_fe_index() + 1);
	  }
	  triangulation.execute_coarsening_and_refinement ();
      }
  ...
  {
    GridGenerator::subdivided_hyper_cube (triangulation, 4, -1., 1., periodic);
  }
  ...
  void LaplaceProblem<dim>::run ()
  {
    ...
    for (unsigned int cycle=0; cycle<num_cycles ; ++cycle)
      {
        auto tot_wtime = std::chrono::high_resolution_clock::now();
        std::cout << "Cycle " << cycle << ':' << std::endl;

        if (cycle == 0)
	{
          create_coarse_grid ();
	  //find_periodic_boundaries();
	}

	wtime = std::chrono::high_resolution_clock::now();
	time = clock();  
        setup_system ();
	time = clock() - time;
	wtime_end = std::chrono::high_resolution_clock::now();
	std::cout <<  "+++ setup system: cpu " << static_cast<double>(time)/CLOCKS_PER_SEC << ", wall " <<  std::chrono::duration<double>(wtime_end-wtime).count() <<  std::endl;
	...
	time = clock() - time;
	wtime_end = std::chrono::high_resolution_clock::now();
	std::cout <<  "+++ assemble system: cpu " << static_cast<double>(time)/CLOCKS_PER_SEC << ", wall " <<  std::chrono::duration<double>(wtime_end-wtime).count() <<  std::endl;
	...
	time = clock() - time;
	wtime_end = std::chrono::high_resolution_clock::now();
	std::cout <<  "+++ solve system: cpu " << static_cast<double>(time)/CLOCKS_PER_SEC << ", wall " <<  std::chrono::duration<double>(wtime_end-wtime).count() <<  std::endl;
	...
	time = clock() - time;
	wtime_end = std::chrono::high_resolution_clock::now();
	std::cout <<  "+++ error calc: cpu " << static_cast<double>(time)/CLOCKS_PER_SEC << ", wall " <<  std::chrono::duration<double>(wtime_end-wtime).count() <<  std::endl;

	std::cout << "Error: " << dof_handler.n_dofs() << " L2 " << L2_error(cycle) << ", DG " << DG_error(cycle)<< std::endl; 

	wtime = std::chrono::high_resolution_clock::now();
	time = clock();  
	postprocess (cycle);
	//postprocess_uniform_h (cycle);
	time = clock() - time;
	wtime_end = std::chrono::high_resolution_clock::now();
	std::cout <<  "+++ postprocessing : cpu " << static_cast<double>(time)/CLOCKS_PER_SEC << ", wall " <<  std::chrono::duration<double>(wtime_end-wtime).count() <<  std::endl;
	auto tot_wtime_end = std::chrono::high_resolution_clock::now();
	std::cout <<  "### total cycle wall time : "  <<  std::chrono::duration<double>(tot_wtime_end-tot_wtime).count() <<  " sec. "<<std::endl;
      }
    error_table();
  }
